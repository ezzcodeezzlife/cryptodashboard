(function () {
  ...
  if (searchInput && navList) {
    var sidebar = document.getElementsByClassName("js-cd-side-nav")[0],
      ...
      mobileNavTrigger =
        document.getElementsByClassName("js-cd-nav-trigger")[0],
      dropdownItems = document.getElementsByClassName(
        "js-cd-item--has-children"
      );
    ...
    for (var i = 0; i < dropdownItems.length; i++) {
      (function (i) {
        dropdownItems[i].children[0].addEventListener(
          "click",
          function (event) {
            if (checkMQ() == "mobile") {
              event.preventDefault();
              ...
              Util.toggleClass(
                item,
                "cd-side__item--expanded",
                !Util.hasClass(item, "cd-side__item--expanded")
              );
            }
          }
        );
    ...
    function moveNavigation() {
      // move searchInput and navList
      var mq = checkMQ();
      if (
        mq == "mobile" &&
        !Util.hasClass(navList.parentNode, "js-cd-side-nav")
      ) {
        detachElements();
        ...
        mq == "desktop" &&
        !Util.hasClass(navList.parentNode, "js-cd-main-header")
      ) {
        detachElements();
        mainHeader.appendChild(navList);
        mainHeader.insertBefore(
          searchInput,
          mainHeader.firstElementChild.nextSibling
        );
      }
      checkSelected(mq);
      resizing = false;
    ...
    function hideHoveredItems() {
      // exit sidebar -> hide dropdown
      var hoveredItems = sidebar.getElementsByClassName("hover");
      for (var i = 0; i < hoveredItems.length; i++)
        Util.removeClass(hoveredItems[i], "hover");
    }

    function checkMQ() {
      // check if mobile or desktop device
      return window
        .getComputedStyle(mainHeader, "::before")
        .getPropertyValue("content")
        .replace(/'|"/g, "");
    }

    function expandSidebarItem() {
      // show dropdown of the selected sidebar item
      Util.addClass(
        sidebar.getElementsByClassName("cd-side__item--selected")[0],
        "cd-side__item--expanded"
      );
    }
    ...
    function checkSelected(mq) {
      ...
      if (mq == "desktop") {
        for (var i = 0; i < dropdownItems.length; i++)
          Util.removeClass(dropdownItems[i], "cd-side__item--expanded");
      }
    }
  }
})();
