...
cron.schedule('0 */12 * * *', function() {
    ...
    coinsToQuery.forEach(function(query){
        request('https://newsapi.org/v2/everything?q=' + query +'&from=2021-10-05&sortBy=publishedAt&apiKey=99e0a262cfe44dc4a105d5c2c1137fcb', function (error, response, body) {
            if (!error && response.statusCode === 200) {
                ...
                JSON.parse(body).articles.forEach(function(obj) {
                    ...
                    try {
                        if(title.includes(query) || obj.content.includes(query) || obj.description.includes(query)){
